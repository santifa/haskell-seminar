...
intToNatg n 
    | n == 0    = (O)
    | n > 0     = S ( intToNatg (n-1) ) 
    | otherwise = error "keine negativen zahlen"
\end{code}
...
div m n 
    ...
    | (subtract m n) == (O) = (O)
    | otherwise = (S (div ( subtract m n ) n ) )
\end{code}
...
\item Kann man eine nicht-strikte Funktion |not'| definieren, die sich auf True und False wie 
    ...
    not hat nur ein Argument und muss für mindestens einen Fall(True oder False) strikt sein
\end{enumerate}
...
\begin{code}
age :: Date -> Date -> Int
age (ay,am,ad) (by,bm,bd) 
    | ay <= by && am <= bm && ad <= bd = by - ay
    | ay <= by && am < bm = by - ay 
    | ay <= by = by - ay - 1
    | otherwise = error "Sie sind noch nicht geboren, komisch"
\end{code}
