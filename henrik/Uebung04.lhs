...
> display (Op (x) k (y)) = "(" ++ display x ++ ops k ++ display y ++ ")"
...
> eval :: (Num a ) => Exp a -> a
> eval (Val a) = a
> eval (Op x Mult y) = (eval x) * (eval y)
> eval (Op x Plus y) = (eval x) + (eval y)
> eval (Op x Minus y) = (eval x) - (eval y)
